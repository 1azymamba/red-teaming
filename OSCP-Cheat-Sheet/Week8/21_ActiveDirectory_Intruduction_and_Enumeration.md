...
## ADの基礎
- ADというサービスは、NW上のすべてのオブジェクトの情報を保持するカタログとして機能する。

- オブジェクトの中には、ユーザ、グループ、マシン、プリンター、共有といったものが含まれる。

**Users** => 最も一般的なオブジェクトタイプ。セキュリティプリンシパルと呼ばれるオブジェクトの一つ。  
ドメインによって認証され、ファイルやプリンターといったリソースへの権限を割り当てられる。  
またユーザには、**サービスユーザ**というものもある。  
サービスユーザは、特手のサービスを実行するために必要な権限のみを持っており、他の通常のユーザとは異なる。

  
**Machines** => AD内の別の種類のオブジェクト。ドメインに参加するコンピュータに対してマシンオブジェクトが作成される。  

- Security Group  
=>いわずもがなグループを作ってADで色々できる。  
デフォルトドメインに存在するグループは以下。
...
=>DCを管理できる。
3. Backup Operators  
=>あらゆるファイルにアクセスできるようになる。
4. Account Operators  
=>ドメイン内の他のアカウントを作成または変更できる。
5. Domain Users  
=>ドメイン内の全ての既存ユーザアカウントが含まれる。
6. Domain Computers  
=>ドメイン内の全ての既存コンピュータが含まれる。
7. Domain Controllers  
=>ドメイン上の全ての既存DCが含まれる。  
[Windowsのドキュメント](https://docs.microsoft.com/en-us/windows/security/identity-protection/access-control/active-directory-security-groups)

- **Delegation(委任)** => 例として、OUでユーザのパスワードをリセットする権限を付与するといった高度なタスクを、ドメイン管理者の介入を必要とせずに実行できる。

- **Group Policy Objects(GPO)** => GPOは、**SYSVOL**と呼ばれるネットワーク共有を介してネットワークに配布される。通常、ドメイン内の全てのユーザは、GPOを定期的に同期するために、ネットワーク経由でこの共有にアクセスできる必要がある。パスは以下で。DC上にある。
```
C:\Windows\SYSVOL\sysvol\
```

- **Tree** => thm.localというドメインがあるとする。これに対してサブドメイン的にuk.thm.localやus.thm.localみたいな風にして統合できる。国ごとに法律なども違うので、それぞれの国に独自のDCを持たせる。  
![alt text](image-1.png)

- **Forest** => 名前空間を別にするのもOK。異なるドメインツリーがあり、異なる名前空間を持つ複数のツリーを同じネットワークに統合したものをForestと呼ぶ。
![alt text](image-2.png)

### DCによる認証

- Windowsドメインを使用する場合、すべての資格情報はDCに保存される。ユーザがドメイン資格情報を使用してサービスへの認証を試みるたびに、サービスはDCに資格情報が正しいか同課を確認してもらうよう要求する必要がある。

- ドメインにおいて、以下のプロトコルが使用される。
1. Kerberos => 現代Windowsではこっちを使用。デフォルトでKerberosのプロトコルを使う。
2. NetNTLM => 互換性を目的として保持されているレガシーの認証プロトコル。

#### Kerberos認証
- サービスにログインするユーザにチケットが割り当てられる。チケットを持っていれば、チケットをサービスに見せることで、認証済みであるということをサービスに対して伝えることができる。

#### NTLM認証
- アプリケーションはDCにユーザから送られてきた認証情報を渡す。

#### LDAP認証
- アプリケーションは自分自身でユーザから送られてきた認証情報をチェックする。