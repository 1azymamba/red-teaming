...
13. サブドメインの列挙で怪しいドメインが見つからなかったか。 
14. Webを見つけてBurpでリクエストとレスポンスを確認した際、curlでも挙動の違いは試したか。特に、GETがダメならPOSTを試すといったアイデアを試したか。
15. Public Exploitのターゲットバージョンが違うという理由で試してないものが無いか。0.1程度の誤差であれば動く可能性もある
```
...
21. ユーザ名が何もわからない場合、以下のように空の情報を渡すことでユーザ名が表示されることがある。
```
rdesktop <target>
```

22. 取得したパスワード等の認証情報がおかしな形式をしている直感を持った場合、cyberchefのmagic機能を使ってb64等に当てはまらないか確認する。
...
42. FTPが動いていて初期侵入で入れなかった場合、侵入後にls -forceでC:\を確認し、そこにあるftpフォルダの中を確認する。
...
11. zipアップロード系の場合、ln -s ../../../etc/passwd hoge.pdfとしてアップロード用のファイルにシンボリックリンクを生成。その後hoge.pdfをzipしてアップロード、さらにhoge.pdfをWebから見に行った時に/etc/passwdを見れないか。
12. PHPのWebの場合、普通のLFIとRFIがダメならb64ラッパーを使ったインクルージョンが機能しないか。
...
21. Sudo version 1.8.23, CVE-2021-4034, PWNKITは引っかからなかったか。
