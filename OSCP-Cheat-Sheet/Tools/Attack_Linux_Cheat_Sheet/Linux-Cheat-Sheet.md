...
1. scpをKaliで受け付けたいときとかに、SCPによるファイル転送を有効化する
```
...
# SSH(ポート転送とか)
1. (SSHクライアント側のOpenSSH7.6以降でのみ利用可能。SSHサーバ側のバージョンは関係ない。)
...
1. -Lオプションでローカルポート転送を行う。-Nでシェルが開かれないようにする。  
接続元の4455ポートに対して、すべてのIPからの通信を許可。  
トンネル経由で送る宛先のIPを172.に指定。  
database_admin@...の部分がSSHサーバになる。  
SSHクライアント => database_admin => 172.16...の順番で転送される。  
この時、database_adminのパスワードを聞かれる。この時、正しいパスワードを入れてもシェルから応答がないが-Nを指定するとこうなるのでおｋ。
```
ssh -N -L 0.0.0.0:4455:172.16.50.217:445 database_admin@10.4.50.215
```

2. SSHリモートポート転送を行うコマンド。  
ターゲット上でSSHクライアントを実行させてポート転送させる。
```
ssh -N -R 127.0.0.1:2345:10.4.50.215:5432 kali@192.1683.59
```

3. SSHリモート動的ポート転送を行うためのコマンド。  
-Rの引数として、ポート番号だけ付与すればいい点が動的じゃない転送と異なる。
```
ssh -N -R 9998 kali@192.168.118.4
```
